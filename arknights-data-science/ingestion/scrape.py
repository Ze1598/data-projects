...
def main():
    ...
    for operator in operator_pages:
        ...
        target_url = operator_pages[operator]
        ...
        def get_stats_per_level(num_levels: int, df: pd.DataFrame) -> pd.DataFrame:
            """Increment the operator level num_levels times and read their stats each time.

            Parameters
            ----------
            num_levels : int 
                Number of levels to scrape.
            df : pd.DataFrame
                Running DF of scraped data.

            Returns
            -------
            pd.DataFrame
                Latest version of the running DF.
            """
            ...
            for i in range(num_levels):
                ...
                # Hp, Atk, Def
                stats_container_elem = driver.find_element_by_id(
                    "stats-container")
                ...
                # Other stats
                other_stats_container_elem = driver.find_element_by_class_name(
                    "other-stat-cell")
                # Split the stat names and values into a list (split at newlines)
                other_stats_split = other_stats_container_elem.text.splitlines()
                # Get the individual stat values
                op_res, op_redeploy, op_cost, op_block, op_atk_interv = other_stats_split[
                    1], other_stats_split[3], other_stats_split[5], other_stats_split[7], other_stats_split[9]

                # for now i will hard code some values, they will be dynamic later when we are not using only SilverAsh's page for development
                row_to_append = {
                    "operator": operator,
                ...
                increase_level_button = driver.find_element_by_class_name(
                    "fa-arrow-right")
...
if __name__ == "__main__":
    ...
    # Create ActionChains object (e.g. scroll to element)
    # actions = ActionChains(driver)
